...
# Labs
lab_url = "http://10.154.0.19/api/labs"
lab_data = {
    "path": "/",
...
network_url = "http://10.154.0.19/api/labs/Test%20-%20Network%20Automation%20Routing.unl/networks"
network_data = {
    "count": "1",
    "visibility": "1",
    "name": "MGMT",
...
def create_instance(name):
    ...
    create_url = 'http://10.154.0.19/api/labs/Test%20-%20Network%20Automation%20Routing.unl/nodes'
    ...
    add_int_url = f'http://10.154.0.19/api/labs/Test%20-%20Network%20Automation%20Routing.unl/nodes/{device_id}/interfaces'
    int_map = '{"0":"1"}'
    ...
    print("Connecting the Interface")
    add_int_url = 'http://10.154.0.19/api/labs/Test%20-%20Network%20Automation%20Routing.unl/nodes/1/interfaces'
    int_map = '{"16":2}'
    connect_int = requests.put(url=add_int_url, data=int_map, headers=headers, cookies=cookies)
    print(connect_int.json())
    
    print("Connecting the Interface")
    add_int_url = 'http://10.154.0.19/api/labs/Test%20-%20Network%20Automation%20Routing.unl/nodes/2/interfaces'
    int_map = '{"16":2}'
    connect_int = requests.put(url=add_int_url, data=int_map, headers=headers, cookies=cookies)
    print(connect_int.json())
    ...
    url = f"http://10.154.0.19/api/labs/Test%20-%20Network%20Automation%20Routing.unl/nodes/{device_id}/start"
    start_api = requests.request("GET", url, headers=headers, cookies=cookies)
    print(start_api.json())

    # Get telnet Port
    url = f"http://10.154.0.19/api/labs/Test%20-%20Network%20Automation%20Routing.unl/nodes"
...
def telnet_initial(telnet_port):
    TELNET_TIMEOUT = 10
    tn = Telnet(host='10.154.0.19', port=telnet_port, timeout=TELNET_TIMEOUT)
    tn.write(b"\n")
    tn.write(b"\n")
    tn.write(b"\n")
    tn.write(b"no\n")
